...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://jack:jack@localhost/jrcleggHW5"
## Provided:
...
class UpdatePriorityForm(FlaskForm):
    newPriority = StringField("What is the new priority for this item?", validators=[Required()])
...
def delete(lst):
    ...
    db.session.commit()
    flash ("Delete list {}".format(list.title))
    return redirect(url_for('all_lists'))
